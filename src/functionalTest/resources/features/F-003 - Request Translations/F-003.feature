...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  @S-003.7 #WLTS-29
  Scenario: must return a successful response when translate operation is requested with an invalid AUTHORIZATION header
    Given a user with [an active solicitor profile]
    When a request is prepared with appropriate values
    And the request [has an invalid IDAM AUTHORIZATION token header]
    And it is submitted to call the [translate] operation of [Translation Service]
    Then a positive response is received
    And the response has all other details as expected.
